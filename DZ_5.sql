...
WHERE 
  profiles.birthday > DATE_SUB(NOW(), INTERVAL 20 YEAR);
...
SELECT 	
	CONCAT(users.firstname, ' ', users.lastname) AS users_name, 
...
(created_at) и найдите разницу дат отправления между соседними сообщениями, получившегося списка. 
(используйте LEAD или LAG)*/

SELECT 	
	body,
    created_at,
    TIMEDIFF(created_at, LAG(created_at) OVER(ORDER BY created_at)) AS next_message
FROM messages
ORDER BY created_at;
